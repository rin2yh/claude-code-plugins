...
- コミットメッセージは why を簡潔に（プロジェクトの規約ファイルに準拠、日本語・適度な敬語）。
...
- 手順7と同じ方法で待機しつつ進捗を追い、**CI が success** になるのを確認する（MCP: `actions_get get_workflow_run` / `actions_list list_workflow_jobs`、gh: `gh run view <run_id>` / `gh run watch <run_id>`）。
...
- 日本語・適度な敬語で報告する（プロジェクトの規約ファイルに準拠）。コミットやPRには why を残す。
